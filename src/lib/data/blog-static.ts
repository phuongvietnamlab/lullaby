 ...
 *
 * Kept only as a bootstrap for an empty `blog_posts` table, matching how rooms,
 * promotions and gallery behave. Anything an admin publishes comes from the DB
 * (see ./blog.ts) and takes precedence.
 *
 * Their `content` is the lightweight markdown-ish format the public renderer
 * originally parsed (## headings, --- rules, - bullets), NOT the HTML the
 * admin editor produces — hence `format: "markdown"`.
 */
