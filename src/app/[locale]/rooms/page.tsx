...
export default async function RoomsPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <RoomsContent locale={locale} />
    </>
  );
}

function RoomsContent({ locale }: { locale: string }) {
                  ...
                  <div className={`space-y-6 ${index % 2 === 1 ? "md:order-1" : ""}`}>
                    <div>
                      <h2 className="font-[family-name:var(--font-heading)] text-3xl md:text-4xl mb-3">
                        {tRoomTypes(`${getRoomI18nKey(room.slug)}.name` as never)}
                      </h2>
                      <p className="text-[var(--color-text-light)] leading-relaxed">
                        {tRoomTypes(`${getRoomI18nKey(room.slug)}.shortDesc` as never)}
                      </p>
                    </div>
