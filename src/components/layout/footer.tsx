...
export async function Footer() {
  ...
  // Contact details and social links come from /admin/settings
  const settings = await getSiteSettings();

  const currentYear = new Date().getFullYear();

  const socials = [
    { href: settings.socialMedia.facebook, label: "Facebook", path: FACEBOOK_PATH },
    { href: settings.socialMedia.instagram, label: "Instagram", path: INSTAGRAM_PATH },
    { href: settings.socialMedia.twitter, label: "X", path: X_PATH },
  ].filter((s) => s.href);
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-6 lg:px-8 py-14 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 sm:gap-12 text-center sm:text-left">
          ...
          <div>
            ...
            {socials.length === 0 ? (
              // No dead "#" links: an unset profile simply is not shown
              <p className="text-sm text-white/40">{t("socialsComingSoon")}</p>
            ) : (
